...
public class CollectionJsonParserTest {
    private CollectionJsonParser parser;

    @Before
    public void setUp() throws Exception {
        parser = new CollectionJsonParser();
    }
    ...
    public void parseSingleItemCollection() throws IOException {
        DefaultCollection collection = (DefaultCollection) parser.parse(new InputStreamReader(getClass().getResourceAsStream("/item.json")));
        assertNotNull(collection);
        assertEquals(URI.create("http://example.org/friends/"), collection.getHref());
    ...
    public void parseValuesExtension() throws IOException {
        DefaultCollection collection = (DefaultCollection) parser.parse(new InputStreamReader(getClass().getResourceAsStream("/value-extension.json")));
        assertNotNull(collection);
        assertEquals(URI.create("http://example.org/friends/"), collection.getHref());
        assertEquals(1, collection.getItems().size());
